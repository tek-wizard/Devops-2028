...
| `kubectl get pods -l app=web-rs` | Filter Pods by label |
| `kubectl scale rs web-rs --replicas=5` | Change the number of Pods |
| `kubectl set image deployment/name container=image` | Start a rolling update |
| `kubectl rollout status deployment/name` | Watch an update finish |
| `kubectl rollout history deployment/name` | The revisions |
| `kubectl rollout undo deployment/name` | Roll back |
| `kubectl describe pod name` | Details and events, for troubleshooting |
